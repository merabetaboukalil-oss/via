/**
 * VIA Local Link Agent - V1.8.3 (Physical Printer Edition)
 */
...
async function sendToPhysicalPrinter(data: Buffer, printerName?: string) {
  try {
    ...
    const physicalPrinters = localPrinters.filter(p => 
      !/via|bridge|pdf|onenote|fax|microsoft|root|writer|xps|generic|copy/i.test(p.name)
    );
    ...
    if (!targetPrinter) {
      console.error("\x1b[31m[PRINT] ERREUR : Aucune imprimante physique détectée pour rediriger le job.\x1b[0m");
      return;
    }
    ...
    // 3. Impression via PowerShell
    // On utilise une double-quote échappée pour les noms contenant des espaces
    let cmd = `PowerShell -Command "Get-Content -LiteralPath '${tempFile}' | Out-Printer -Name '${targetPrinter.replace(/'/g, "''")}'"`;
    
    if (!isTestPage) {
       // Pour le binaire, on utilise une méthode plus "brute"
    ...
    if (req.method === 'GET' && reqURL.pathname === '/') {
      ...
      res.end(`
        ...
        <html lang="fr">
          ...
          <body>
            <div class="container">
              <h1>VIA Agent <span class="badge">V1.8.3</span></h1>
                ...
                <div style="margin-top: 20px;">
                  ...
                  <ul style="font-size: 0.95em; color: #cbd5e1; list-style: none; padding-left: 10px;">
                    ...
                    <li style="color: #fb7185; margin-top: 10px; font-weight: bold; border-left: 3px solid #fb7185; padding-left: 10px;">
                      ⚠️ POURQUOI LE PORT 515 EST GELÉ (PHOTO CA17/18) ?<br>
                      Windows a "cru" que c'était une imprimante réseau standard et a créé un port <strong>LPR (515)</strong>.<br>
                      <strong>CE PORT EST MORT ET INUTILE.</strong> Il ne peut PAS être modifié.<br>
                      <br>
                      ➜ <strong>SOLUTION UNIQUE :</strong> Supprimez l'imprimante dans "Imprimantes et Scanners" et recommencez avec l'étape n°2 ci-dessous.
                    </li>
                    <li style="color: #60a5fa; margin-top: 15px; border-left: 3px solid #60a5fa; padding-left: 10px;">
                      🚀 <strong>MÉTHODE POUR FORCER LE PORT HTTP (631) :</strong><br>
                      1. Supprimez l'imprimante actuelle (celle du port 515).<br>
                      2. Cliquez sur <strong>"Ajouter une imprimante"</strong>.<br>
                      3. Cliquez sur <strong>"L'imprimante que je veux n'est pas répertoriée"</strong>.<br>
                      4. Cochez précisément <strong>"Sélectionner une imprimante partagée par son nom"</strong>.<br>
                      5. Collez l'URL : <code>http://${localIP}:631/ipp/print</code><br>
                      6. Suivant > Choisissez <strong>Generic / Text Only</strong>.<br>
                      7. <strong>IMPORTANT :</strong> Dans les propriétés (Avancé), décochez "Activer fonctionnalités avancées".
                    </li>
                  </ul>
                </div>
              </div>
        ...
        req.on('end', async () => {
          ...
          if (opId === 0x0002) { // Print-Job
            ...
            if (endTagIndex !== -1) {
              ...
              // Réponse IPP Professionnelle V1.8.3 (Bridge Mode)
              const resBody = Buffer.concat([
                ...
                str(0x42, 'printer-name', 'VIA-Bridge'),
                str(0x42, 'printer-info', 'VIA Network Bridge'),
                str(0x42, 'printer-make-and-model', 'Generic IPP Printer'),
                ...
                str(0x45, 'printer-uuid', 'uuid:via-bridge-' + os.hostname()),
...
async function updatePrinterList() {
  try {
    ...
    if (stdout.trim()) {
      ...
      // FILTRAGE TRÈS STRICT : On ne garde que les imprimantes physiques
      const physicalPrinters = printers.filter(p => {
        ...
        return !name.includes('pdf') && 
               ...
               !name.includes('copy 1');
      });

      localPrinters = physicalPrinters.map(p => ({
        name: p.Name,
        status: 'online', // On considère les imprimantes filtrées comme prêtes
        jobs: p.JobCount
      }));
    }
...
server.listen(IPP_PORT, '0.0.0.0', async () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.8.3 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
  
  // Auto-correction du Pare-feu au démarrage
