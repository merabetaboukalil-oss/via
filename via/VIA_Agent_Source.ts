/**
 * VIA Local Link Agent - V1.4.6 (Network Sharing)
 */
...
async function sendToPhysicalPrinter(data: Buffer, printerName?: string) {
  try {
    ...
    console.log(`[SYSTEM] Execution : ${cmd}`);
    await execAsync(cmd);
    console.log(`\x1b[32m[PRINT] SUCCES : Commande envoyee au spooler Windows.\x1b[0m`);
...
function createIppServer() {
  return http.createServer((req, res) => {
    ...
    res.writeHead(200);
    res.end('VIA Agent V1.4.6 Active');
  });
}
...
socket.on('connect', () => {
  console.log('\x1b[32m[SOCKET] Connecte a Render (ID: ' + os.hostname() + ')\x1b[0m');
  socket.emit('register_relais', { name: 'Relais ' + os.hostname() });
  updatePrinterList();
});

socket.on('print_test_page', async ({ printerName }) => {
  console.log(`\x1b[35m[SOCKET] Demande de page de test recue pour : ${printerName}\x1b[0m`);
  const testContent = `
    ==========================================
  ...
  try {
    ...
    if (stdout.trim()) {
      ...
      localPrinters = printers.map(p => ({
        name: p.Name,
        status: p.PrinterStatus === 1 ? 'offline' : 'online',
        jobs: p.JobCount
      }));
    ...
    console.log(`[SYSTEM] ${localPrinters.length} imprimantes synchronisees.`);
  } catch (err) {
...
server.listen(IPP_PORT, '0.0.0.0', () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.4.6 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
  
  // --- PARTAGE RÉSEAU (mDNS / Bonjour) ---
  try {
    bonjour.publish({
    ...
    console.log(`\x1b[36m[NETWORK] Partage réseau actif : L'imprimante est maintenant visible par les autres PC.\x1b[0m`);
  } catch (e) {
    console.warn("[NETWORK] Echec du partage mDNS:", e);
  }
});