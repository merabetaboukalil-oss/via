/**
 * VIA Local Link Agent - V1.6.1 (Universal Windows Sync Pro)
 * 
 * INSTALLATION :
    ...
    if (req.method === 'GET' && reqURL.pathname === '/') {
      ...
      res.end(`
        ...
        <html lang="fr">
          ...
          <body>
            <div class="container">
              <h1>VIA Agent <span class="badge">V1.6.1</span></h1>
    ...
    if (reqURL.pathname === '/ipp/print') {
      ...
      if (req.method === 'POST') {
        ...
        req.on('end', async () => {
          ...
          // Réponse IPP ultra-complète pour satisfaire Windows 10/11
          const resBody = Buffer.concat([
            ...
            str(0x42, 'printer-name', 'VIA_Printer_Bridge'),
            str(0x42, 'printer-info', 'VIA Network Printer Bridge'),
            str(0x42, 'printer-make-and-model', 'VIA Universal IPP Printer'),
            ...
            str(0x44, 'ipp-versions-supported', '1.0,1.1,2.0'),
            str(0x49, 'document-format-supported', 'application/octet-stream,image/pwg-raster,application/pdf'),
            str(0x45, 'printer-uuid', 'uuid:via-bridge-' + os.hostname().replace(/[^a-zA-Z0-9]/g, '-')),
            ...
            // Operations: 2(Print), 4(Validate), 10(Get-Jobs), 11(Get-Printer-Attributes)
            Buffer.from([0x23, 0x00, 0x14, ...Buffer.from('operations-supported'), 0x00, 0x04, 0x00, 0x00, 0x00, 0x02]),
            Buffer.from([0x23, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x04]),
            Buffer.from([0x23, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0a]),
            Buffer.from([0x23, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x0b]),
            str(0x44, 'printer-state-reasons', 'none'),
...
server.listen(IPP_PORT, '0.0.0.0', async () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.6.0 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
