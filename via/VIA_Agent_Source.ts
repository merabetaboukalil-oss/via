/**
 * VIA Local Link Agent - V1.6.5 (Enterprise Sync Pro)
 */
    ...
    if (req.method === 'GET' && reqURL.pathname === '/') {
      ...
      res.end(`
        ...
        <html lang="fr">
          ...
          <body>
            <div class="container">
              <h1>VIA Agent <span class="badge">V1.6.3</span></h1>
              ...
              <div class="success-step">
                ...
                <div style="margin-top: 20px;">
                  ...
                  <ul style="font-size: 0.95em; color: #cbd5e1; list-style: none; padding-left: 10px;">
                    ...
                    <li>➜ Collez l'URL ci-dessus et suivez les étapes.</li>
                  </ul>
                </div>
              </div>
            </div>
    ...
    if (reqURL.pathname === '/ipp/print') {
      ...
      if (req.method === 'POST') {
        ...
        req.on('end', async () => {
          ...
          // Réponse IPP Professionnelle V1.6.4 (Precision Encoding)
          const resBody = Buffer.concat([
            ...
            str(0x42, 'printer-name', 'VIA-Canon-Bridge'),
            str(0x42, 'printer-info', 'VIA Network Bridge'),
            str(0x42, 'printer-make-and-model', 'Canon LBP3010 IPP Bridge'),
            str(0x42, 'printer-location', 'Network-VIA'),
            str(0x47, 'charset-configured', 'utf-8'),
            multiStr(0x47, 'charset-supported', ['utf-8', 'us-ascii']),
            str(0x48, 'natural-language-configured', 'en-us'),
            multiStr(0x48, 'generated-natural-language-supported', ['en-us']),
            str(0x45, 'printer-more-info', baseURL),
            multiStr(0x45, 'printer-uri-supported', [baseURL + '/ipp/print', baseURL.replace('http', 'ipp') + '/ipp/print']),
            str(0x44, 'uri-security-supported', 'none'),
            str(0x44, 'uri-authentication-supported', 'none'),
            multiStr(0x44, 'ipp-versions-supported', ['1.0', '1.1', '2.0', '2.1']),
            str(0x49, 'document-format-default', 'application/octet-stream'),
            multiStr(0x49, 'document-format-supported', ['application/octet-stream', 'application/pdf', 'image/pwg-raster', 'image/jpeg']),
            str(0x45, 'printer-uuid', 'uuid:550e8400-e29b-41d4-a716-' + os.hostname().substring(0, 12).padEnd(12, '0')),
            Enum('printer-state', 3), // 3 = idle
            bool('printer-is-accepting-jobs', true),
            bool('color-supported', false),
            str(0x44, 'pdl-override-supported', 'attempted'),
            str(0x44, 'sides-default', 'one-sided'),
            multiStr(0x44, 'sides-supported', ['one-sided']),
            str(0x44, 'media-default', 'iso_a4_210x297mm'),
            multiStr(0x44, 'media-ready', ['iso_a4_210x297mm']),
            multiStr(0x44, 'media-supported', ['iso_a4_210x297mm', 'na_letter_8.5x11in']),
            multiEnum('operations-supported', [0x0002, 0x0004, 0x0005, 0x0006, 0x0008, 0x0009, 0x000a, 0x000b]),
            multiStr(0x44, 'printer-state-reasons', ['none']),
            multiEnum('finishings-supported', [3]), // 3 = none
            Enum('finishings-default', 3),
            multiEnum('print-quality-supported', [4]), // 4 = normal
            Enum('print-quality-default', 4),
            multiEnum('orientation-requested-supported', [3, 4]), // 3=portrait, 4=landscape
            Enum('orientation-requested-default', 3),
            int('queued-job-count', 0),
            int('printer-up-time', Math.floor(os.uptime())),
            int('pages-per-minute', 12),
...
server.listen(IPP_PORT, '0.0.0.0', async () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.6.3 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
