/**
 * VIA Local Link Agent - V1.5.3 (Security Update)
 * 
 * INSTALLATION :
...
async function fixWindowsFirewall() {
  try {
    ...
    // On utilise Start-Process avec RunAs pour demander les droits admin (Popup Windows)
    const cmd = `PowerShell -Command "Start-Process PowerShell -Verb RunAs -ArgumentList 'New-NetFirewallRule -DisplayName \\"VIA Printer Bridge\\" -Direction Inbound -LocalPort 631 -Protocol TCP -Action Allow -ErrorAction SilentlyContinue'"`;
    await execAsync(cmd);
    return true;
  } catch (e) {
  ...
  return http.createServer((req, res) => {
    ...
    const html = `
      ...
      <html lang="fr">
        ...
        <body>
          <div class="container">
            <h1>VIA Agent <span class="badge">V1.5.3</span></h1>
...
server.listen(IPP_PORT, '0.0.0.0', async () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.5.3 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
