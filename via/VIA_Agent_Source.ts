/**
 * VIA Local Link Agent - V1.4.2 (Fix Connection & ID)
 */
...
async function sendToPhysicalPrinter(data: Buffer) {
  try {
    ...
    console.log(`\x1b[36m[PRINT] Envoi vers : ${targetPrinter}\x1b[0m`);
    ...
    await execAsync(cmd);
    console.log(`\x1b[32m[PRINT] SUCCÈS : Document envoyé à l'imprimante.\x1b[0m`);
...
function createIppServer() {
  return http.createServer((req, res) => {
    ...
    res.writeHead(200);
    res.end('VIA Agent V1.4.2 Active');
  });
}
...
server.listen(IPP_PORT, '0.0.0.0', () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.4.2 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
});