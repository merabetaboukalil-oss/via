/**
 * VIA Local Link Agent - V1.4.4 (Out-Printer Strategy)
 * 
...
async function sendToPhysicalPrinter(data: Buffer, printerName?: string) {
  try {
    ...
    console.log(`\x1b[33m[CONSEIL] Si l'imprimante ne repond pas, verifiez qu'elle n'est pas en 'Pause' dans Windows.\x1b[0m`);
  }
}
...
function createIppServer() {
  return http.createServer((req, res) => {
    ...
    res.writeHead(200);
    res.end('VIA Agent V1.4.4 Active');
  });
}

// --- CONNEXION AVEC IDENTIFICATION ---
...
server.listen(IPP_PORT, '0.0.0.0', () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.4.4 pret sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
});