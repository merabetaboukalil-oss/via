/**
 * VIA Local Link Agent - V1.5.8 (IP Detection & Network Fix)
 * 
 * INSTALLATION :
      ...
      <html lang="fr">
        <head>
          ...
          <style>
            ...
            .warning-box { background: rgba(239, 68, 68, 0.1); border: 1px solid #ef4444; padding: 15px; border-radius: 8px; margin-bottom: 20px; color: #fca5a5; font-size: 0.9em; }
            ...
            .url-box { background: #020617; padding: 10px; border-radius: 6px; font-family: monospace; color: #3b82f6; border: 1px solid #1e293b; display: block; margin: 10px 0; font-weight: bold; }
        ...
        <body>
          <div class="container">
            <h1>VIA Agent <span class="badge">V1.5.8</span></h1>
            ...
            ${localPrinters.length === 0 ? '<p style="color: #64748b;">Aucune imprimante détectée...</p>' : localPrinters.map(p => `
              <div class="printer-card">
                <div class="printer-info-row">
                  ...
                  <div class="actions">
                    <button class="btn-test" onclick="doAction('/api/test', 'printer=${encodeURIComponent(p.name)}')">Page de Test Local</button>
                    <button class="btn-share" onclick="doAction('/api/sync')">Forcer Synchro</button>
                  </div>
                </div>
              </div>
            `).join('')}
            ...
            <div class="help-box">
              <h3 style="color: #3b82f6; margin-top: 0;">🛠 Diagnostic de Connexion</h3>
              
              <div class="warning-box">
                <strong>CRITIQUE :</strong> Votre réseau Windows doit être en mode <strong>"Privé"</strong>. S'il est en "Public", Windows bloquera TOUTE connexion entrante.
              </div>

              <p><strong>1. Adresse IP Recommandée (Plus fiable) :</strong></p>
              <div class="url-box">http://${localIP}:631/ipp/print</div>
              
              <p style="margin-top: 20px;"><strong>2. En cas de refus de connexion :</strong></p>
              <button class="btn-fix" onclick="doAction('/api/fix-firewall')">Ré-ouvrir le Pare-feu Windows</button>
              
              <ul style="font-size: 0.9em; color: #94a3b8; margin-top: 15px;">
                <li>Vérifiez que le 2ème PC est sur le même Wi-Fi.</li>
                <li>Tapez l'adresse IP ci-dessus dans le navigateur du 2ème PC : si une page s'affiche, c'est que le réseau est bon !</li>
              </ul>
            </div>
          </div>
...
server.listen(IPP_PORT, '0.0.0.0', async () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.5.8 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
