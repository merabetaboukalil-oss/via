/**
 * VIA Local Link Agent - V1.5.6 (Firewall Naming Fix)
 * 
 * INSTALLATION :
...
async function fixWindowsFirewall() {
  try {
    ...
    // Utilisation d'un nom sans espaces pour éviter les problèmes de guillemets imbriqués
    const cmd = `PowerShell -Command "Start-Process PowerShell -Verb RunAs -ArgumentList '-NoExit','New-NetFirewallRule -DisplayName VIA_Printer_Bridge -Direction Inbound -LocalPort 631 -Protocol TCP -Action Allow'"`;
    await execAsync(cmd);
    return true;
  } catch (e) {
  ...
  return http.createServer((req, res) => {
    ...
    const html = `
      ...
      <html lang="fr">
        ...
        <body>
          <div class="container">
            <h1>VIA Agent <span class="badge">V1.5.6</span></h1>
...
server.listen(IPP_PORT, '0.0.0.0', async () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.5.6 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
