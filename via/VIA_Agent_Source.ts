/**
 * VIA Local Link Agent - V1.6.3 (Stable Sync Pro)
 */
    ...
    if (reqURL.pathname === '/ipp/print') {
      ...
      if (req.method === 'POST') {
        ...
        req.on('end', async () => {
          ...
          // Réponse IPP Professionnelle V1.6.2 (Strict Encoding)
          const resBody = Buffer.concat([
            ...
            str(0x42, 'printer-name', 'VIA_Printer'),
            ...
            multiStr(0x47, 'charset-supported', ['utf-8']),
            ...
            Buffer.from([0x23, 0x00, 0x0d, ...Buffer.from('printer-state'), 0x00, 0x04, 0x00, 0x00, 0x00, 0x03]), // 3 = idle
            Buffer.from([0x22, 0x00, 0x1a, ...Buffer.from('printer-is-accepting-jobs'), 0x00, 0x01, 0x01]), // true
            Buffer.from([0x22, 0x00, 0x0f, ...Buffer.from('color-supported'), 0x00, 0x01, 0x00]), // false (N&B)
            ...
            Buffer.from([0x21, 0x00, 0x10, ...Buffer.from('queued-job-count'), 0x00, 0x04, 0x00, 0x00, 0x00, 0x00]),
