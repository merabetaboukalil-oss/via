/**
 * VIA Local Link Agent - V1.8.4 (Ultimate Bridge)
 */
...
async function triggerTestPage(printerName: string) {
  ...
  if (!target) {
    console.error("[LOCAL-DASH] Erreur : Aucune imprimante cible pour le test.");
    return;
  }
...
async function sendToPhysicalPrinter(data: Buffer, printerName?: string) {
  try {
    ...
    if (!targetPrinter) {
      console.error("\x1b[31m[PRINT] ERREUR : Aucune imprimante détectée.\x1b[0m");
      return;
    }
    ...
    let cmd = `PowerShell -Command "Out-Printer -Name '${targetPrinter.replace(/'/g, "''")}' -InputObject (Get-Content -LiteralPath '${tempFile}' -Raw)"`;
    ...
    console.log(`\x1b[32m[PRINT] SUCCÈS : Transmis à l'imprimante.\x1b[0m`);
    ...
    if (req.method === 'GET' && reqURL.pathname === '/') {
      ...
      res.end(`
        ...
        <html lang="fr">
          ...
          <body>
            <div class="container">
              <h1>VIA Agent <span class="badge">V1.8.4</span></h1>
...
async function updatePrinterList() {
  try {
    ...
    if (stdout.trim()) {
      ...
      // FILTRAGE MINIMALE : On ne garde que les imprimantes physiques probables, mais on reste souple
...
server.listen(IPP_PORT, '0.0.0.0', async () => {
  console.log(`\x1b[32m[NETWORK] Agent V1.8.4 prêt sur http://localhost:${IPP_PORT}/ipp/print\x1b[0m`);
