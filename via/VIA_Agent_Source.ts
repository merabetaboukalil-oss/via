/**
 * VIA Local Link Agent - V1.6.0 (Universal Windows Sync)
 * 
 * INSTALLATION :
...
function createIppServer() {
  return http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && reqURL.pathname === '/') {
      ...
      res.end(`
        ...
        <html lang="fr">
          ...
          <body>
            <div class="container">
              <h1>VIA Agent <span class="badge">V1.6.0</span></h1>
    ...
    if (req.method === 'POST' && reqURL.pathname === '/ipp/print') {
      const chunks: any[] = [];
      ...
      req.on('end', async () => {
        ...
        // Réponse IPP survitaminée pour Windows (UUID, multiple versions, etc.)
        const resBody = Buffer.concat([
          ...
          str(0x42, 'printer-make-and-model', 'VIA Universal Printer'),
          ...
          str(0x49, 'document-format-supported', 'application/octet-stream,application/pdf,image/jpeg'),
          str(0x45, 'printer-uuid', 'uuid:via-bridge-' + os.hostname().replace(/\s+/g, '-')),
          ...
          Buffer.from([0x23, 0x00, 0x14, ...Buffer.from('operations-supported'), 0x00, 0x04, 0x00, 0x00, 0x00, 0x02]), // Print-Job
