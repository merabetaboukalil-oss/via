...
export default function PrinterModal({ isOpen, onClose, pcs, activeSocket }: { isOpen: boolean, onClose: () => void, pcs: any[], activeSocket?: any }) {
    ...
    if (isOpen) {
      ...
      const existingRelais = pcs
        ...
        .map(pc => ({
          id: pc.id,
          name: pc.name,
          printers: pc.printers || []
        }));
      
      // If we just installed one and it's not in the list yet, add a simulated entry for immediate feedback
      if (showSuccess && existingRelais.length === 0) {
        existingRelais.push({
          id: 'relais-simulated',
          name: 'Mon Relais VIA (Actif)',
          printers: [
            { id: 'p1', name: 'Canon G3010 Series (USB)', type: 'usb', status: 'online' }
          ]
        });
      }
  ...
  const handleInstallAgent = () => {
    ...
    setTimeout(async () => {
      ...
      // Register this PC as a relais
      let socket = activeSocket;
      
      if (!socket || (socket as any).io?.opts?.query?.role !== 'pc') {
        socket = getSocket('pc', { 
          name: 'Agent VIA Local',
          id: 'relais-' + Math.random().toString(36).substring(7)
        });
      }
      
      if (socket) {
        socket.emit('register_relais', { 
          name: 'Mon Relais VIA (Actif)'
        });
        
        // Initial simulation - Start with a quick scan
        socket.emit('update_printers', [
          { id: 'p1', name: 'Canon G3010 Series (USB)', type: 'usb', status: 'online' }
        ]);
      }
    }, 5000);
  ...
  const detectRealPrinters = async () => {
    try {
      ...
      if (device) {
        ...
        // Update the server with the REAL printer
        let socket = activeSocket || getSocket('pc');
        if (socket) {
          socket.emit('update_printers', [newPrinter]);
        }
      }
    } catch (err) {
